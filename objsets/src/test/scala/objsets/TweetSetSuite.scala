...
class TweetSetSuite extends FunSuite {
  ...
  test("include: duplicate") {
    new TestSets {
      assert(size(set2.incl(new Tweet("a", "a body", 20))) === 1)
    }
  }
  ...
  test("mostRetweeted: with 1 element set") {
    new TestSets {
      assert(set2.mostRetweeted.text === "a body")
    }
  }

  test("mostRetweeted: with multi element set") {
    new TestSets {
  ...
  test("descending: set2") {
    new TestSets {
      ...
      assert(!trends.isEmpty)
      assert(trends.head.text === "a body")
    }
  }
